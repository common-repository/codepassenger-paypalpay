<?php
/**
 * Wordpress Plugin Settings class
 * 
...
if ( !class_exists( 'Ppdp_wordPress_Plugin_Settings_class' ) ):

class Ppdp_wordPress_Plugin_Settings_class {
	...
	public function __construct( $settings_fields, $settings_option_page, $settings_prefix ) {
	   if( !is_file($settings_fields) ) return;
		...
		$this->settings = array();
		$this->settings = apply_filters( $this->settings_option_page.'_fields', $this->settings );
		if( !is_array($this->settings) ){
			return new WP_Error( 'broke', esc_html__( 'Paypal settings must be an array','ppdp' ) );
		}
	...
	{
		wp_register_style('ppdp-admin-style',plugins_url('/css/ppdp-admin-style.css',__FILE__));
		wp_enqueue_style('ppdp-admin-style'); 
		wp_enqueue_style( 'wp-color-picker' );

        wp_enqueue_media();
        wp_enqueue_script( 'wp-color-picker' );
		wp_enqueue_script( 'ppdp-admin-script',plugins_url('/js/ppdp-admin-script.js',__FILE__),array('jquery'),false,false);
	}
	...
	public function register_settings() {
		if( !empty( $this->settings ) ):
			foreach( $this->settings as $section_id => $section_data ):
				...
				add_settings_section( $section_id, $section_data['title'], array( $this, 'display_section' ), $this->settings_option_page.'_'.$section_id );
	...
	public function display_field( $args ) {
		...
		switch( $args['type'] ) {
			...
			case 'select':
				$html .= '<select name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $args['id'] ) . '" class="'.  esc_attr($class) . '" >';
				foreach( $args['options'] as $k => $v ) {
					$selected = false;
					if( $k == $value ) {
						$selected = true;
			...
			case 'multiple_select':
				$html .= '<select name="' . esc_attr( $field_name ) . '[]" id="' . esc_attr( $args['id'] ) . '" multiple="multiple">';
				foreach( $args['options'] as $k => $v ) {
					$selected = false;
					if(!empty($value)):
						if(in_array( $k, $value )):
